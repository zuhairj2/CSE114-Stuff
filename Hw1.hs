...
digitsOfInt n | n <= 0 = [] | otherwise = digitsOfInt (n `div` 10) ++ [n `mod` 10]
...
digitsOfInts [] = []
digitsOfInts (x:xs)
  | x > 0     = digitsOfInt x ++ digitsOfInts xs
  | otherwise = digitsOfInts xs
...
validateCardNumber n =
  sumList (digitsOfInts (doubleEveryOther (digitsOfInt n))) `mod` 10 == 0